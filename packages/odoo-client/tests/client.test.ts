...
describe('OdooClient', () => {
  ...
  // TC-15: probe() throws OdooAuthError when not authenticated
  it('probe() throws OdooAuthError when not authenticated', async () => {
    const client = new OdooClient(CONFIG);
    await expect(client.probe()).rejects.toBeInstanceOf(OdooAuthError);
  });

  // TC-16: probe() with all 8 sub-queries failing returns ProbeResult with all error fields
  it('probe() with all 8 sub-queries failing returns all { error: string } fields', async () => {
    const client = await authenticatedClient();
    // All jsonRpc calls throw
    mockJsonRpc.mockRejectedValue(new Error('RPC failure'));

    const result = await client.probe();

    // No exception thrown, all 8 fields are { error: string }
    expect('error' in result.modules).toBe(true);
    expect('error' in result.reports).toBe(true);
    expect('error' in result.serverActions).toBe(true);
    expect('error' in result.companies).toBe(true);
    expect('error' in result.currencies).toBe(true);
    expect('error' in result.fiscalYear).toBe(true);
    expect('error' in result.language).toBe(true);
    expect('error' in result.locale).toBe(true);

    expect((result.modules as { error: string }).error).toBe('RPC failure');
    expect((result.language as { error: string }).error).toBe('RPC failure');
  });

  // TC-17: probe() with 3 failing returns 3 errors and 5 populated fields
  it('probe() with 3 failing sub-queries populates 5 fields normally', async () => {
    const client = await authenticatedClient();

    // searchRead calls: modules(1), reports(2), serverActions(3), companies(4), currencies(5)
    // read calls: fiscalYear(6), language(7), locale(8)
    // We fail reports(2), serverActions(3), and language(7), succeed the rest.

    // Track call count to differentiate
    let callCount = 0;
    mockJsonRpc.mockImplementation(
      (_url: unknown, _ep: unknown, params: Record<string, unknown>) => {
        callCount++;
        ...
        if (model === 'ir.module.module') {
          // modules — succeed
          return Promise.resolve([{ id: 1, name: 'sale', latest_version: '17.0.1' }]);
        }
        if (model === 'ir.actions.report') {
          // reports — fail
          return Promise.reject(new Error('reports error'));
        }
        if (model === 'ir.actions.server') {
          // serverActions — fail
          return Promise.reject(new Error('serverActions error'));
        }
        ...
        if (model === 'res.currency') {
          // currencies — succeed
          return Promise.resolve([{ id: 1, name: 'USD', symbol: '$' }]);
        }
        if (model === 'res.company' && method === 'read') {
          // fiscalYear — succeed
          return Promise.resolve([{ id: 1, fiscalyear_last_day: 31, fiscalyear_last_month: 12 }]);
        }
        if (model === 'res.users' && method === 'read') {
          const args = params.args as unknown[][];
          const fields = (params.kwargs as Record<string, unknown>).fields as string[];
          if (fields && fields[0] === 'lang') {
            // language — fail
            return Promise.reject(new Error('language error'));
          }
          // locale (tz) — succeed
          return Promise.resolve([{ id: SESSION.uid, tz: 'UTC' }]);
        }
        return Promise.resolve([]);
      },
    );
    ...
    expect('date_from' in result.fiscalYear).toBe(true);
    ...
    expect('error' in result.serverActions).toBe(true);
    expect('error' in result.language).toBe(true);
    expect((result.reports as { error: string }).error).toBe('reports error');
    expect((result.serverActions as { error: string }).error).toBe('serverActions error');
    expect((result.language as { error: string }).error).toBe('language error');

    void callCount; // used to aid debug; not a direct assertion
  });

  // TC-18: probe() fiscalYear populates date_from / date_to for current year
  it('probe() fiscalYear returns current-year date_from and date_to', async () => {
    const client = await authenticatedClient();
    const year = new Date().getFullYear();

    // All succeed; only check fiscal year output
    mockJsonRpc.mockImplementation(
      (_url: unknown, _ep: unknown, params: Record<string, unknown>) => {
        const model = params.model as string;
        const method = params.method as string;

        if (model === 'ir.module.module') return Promise.resolve([]);
        if (model === 'ir.actions.report') return Promise.resolve([]);
        if (model === 'ir.actions.server') return Promise.resolve([]);
        if (model === 'res.company' && method === 'search_read') return Promise.resolve([]);
        if (model === 'res.currency') return Promise.resolve([]);
        if (model === 'res.company' && method === 'read')
          return Promise.resolve([{ id: 1, fiscalyear_last_day: 31, fiscalyear_last_month: 12 }]);
        if (model === 'res.users') {
          const fields = (params.kwargs as Record<string, unknown>).fields as string[];
          if (fields?.[0] === 'lang') return Promise.resolve([{ id: 7, lang: 'en_US' }]);
          return Promise.resolve([{ id: 7, tz: 'UTC' }]);
        }
        return Promise.resolve([]);
      },
    );
    ...
    const result = await client.probe();
    const fy = result.fiscalYear as { date_from: string; date_to: string };
    expect(fy.date_from).toBe(`${year}-01-01`);
    expect(fy.date_to).toBe(`${year}-12-31`);
  });
});
